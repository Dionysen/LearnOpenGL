...
void initOpenGL()
{
  ...
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER,
                  GL_NEAREST); // 缩小的纹理过滤方式
  // 可以设置为多级渐变纹理过滤：
  // glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER,
  // GL_NEAREST_MIPMAP_LINEAR);
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER,
                  GL_NEAREST); // 放大时的纹理过滤方式
...
void render()
{
  ...
  // identity matrix first glm::mat4 view = glm::mat4(1.0f); glm::mat4
  // projection = glm::mat4(1.0f); model = glm::rotate(model, rotation,
  // glm::vec3(0.5f, 1.0f, 0.0f)); view = glm::translate(view, glm::vec3(0.0f,
  // 0.0f, -3.0f)); projection = glm::perspective(glm::radians(45.0f),
  ...
  // the main loop only once. ourShader.setMat4("projection", projection);
